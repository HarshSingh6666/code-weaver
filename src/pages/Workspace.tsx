    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3">
        <Button asChild variant="hero" size="sm" className="w-full">
          <Link to="/upload" onClick={onClose}><Plus /> New Repository</Link>
        </Button>
      </div>
        ...
        {user && (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[10px] font-mono text-muted-foreground truncate">via {user.provider}</div>
            </div>
            <Button
              variant="ghost"
              size="icon"
...
const FlowchartPanel = ({ activeMsg }: { activeMsg: Msg | null }) => (
  <div className="flex flex-col h-full">
    <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-border/50 glass shrink-0">
      <div>
    ...
    <div className="flex-1 overflow-auto p-4 md:p-6">
      {activeMsg?.diagram ? (
        <motion.div
          ...
          className="glass-strong rounded-2xl p-4 md:p-6 min-h-[300px] flex items-center justify-center"
        >
...
const Workspace = () => {
  ...
  const ChatPanel = (
    <div className="flex flex-col h-full min-w-0">
      <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-border/50 glass shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
            ...
            <SheetContent side="left" className="w-72 p-0 glass-strong border-border/50">
              <SidebarContent onClose={() => setMobileNavOpen(false)} />
            </SheetContent>
          </Sheet>
          <Button asChild variant="ghost" size="icon" className="md:hidden shrink-0">
            <Link to="/"><ArrowLeft /></Link>
          </Button>
          <div className="min-w-0">
            <div className="text-sm font-display font-semibold truncate">Code Conversation</div>
            <div className="text-[10px] font-mono text-muted-foreground flex items-center gap-1.5 truncate">
              <Folder className="h-3 w-3 shrink-0" />
              <span className="truncate">~/projects/legacy-banking-app</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="ghost" size="sm" className="hidden sm:inline-flex"><Download className="h-4 w-4" /></Button>
          <Button variant="glow" size="sm"><Sparkles /> <span className="hidden sm:inline">New chat</span></Button>
        </div>
      </header>

      <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 sm:px-4 md:px-8 py-6 space-y-6">
      ...
      {messages.length <= 1 && (
        <div className="px-3 sm:px-4 md:px-8 pb-3 flex flex-wrap gap-2">
          {suggestions.map((s) => (
            <button
              key={s}
              onClick={() => send(s)}
      ...
      <div className="p-3 sm:p-4 md:px-8 md:pb-6 border-t border-border/50">
        <div className="glass-strong rounded-2xl flex items-end gap-2 p-2">
          <textarea
            ...
            className="flex-1 bg-transparent resize-none px-3 sm:px-4 py-3 text-sm focus:outline-none placeholder:text-muted-foreground max-h-40 min-w-0"
          />
          <Button variant="hero" size="icon" onClick={() => send()} disabled={thinking || !input.trim()}>
            <Send />
          </Button>
        </div>
        <div className="text-[10px] font-mono text-muted-foreground mt-2 text-center hidden sm:block">
  ...
  return (
    <div className="relative h-screen flex overflow-hidden">
      ...
      {/* Desktop sidebar */}
      <aside className="relative z-10 hidden md:flex flex-col w-60 lg:w-64 border-r border-border/50 glass shrink-0">
        <SidebarContent />
      </aside>
      ...
      <div className="relative z-10 flex-1 min-w-0">
        ...
        >
          ...
          <ResizableHandle withHandle className="bg-border/50 hover:bg-primary/40 transition-colors" />
          <ResizablePanel defaultSize={40} minSize={20}>
            <FlowchartPanel activeMsg={activeMsg} />
          </ResizablePanel>
        </ResizablePanelGroup>
      </div>
    </div>
